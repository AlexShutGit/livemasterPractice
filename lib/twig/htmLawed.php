...
require_once __DIR__ . '/mbStringUtils.php';
...
if($x == '-*'){$e = array();}
elseif(mb_strpos($x, '*') === false){$e = array_flip(explode(',', $x));}
else{
 if(isset($x[1])){
  ...
  foreach($m as $v){
   ...
   if($v[0] == '-' && isset($e[($v = mb_substr($v, 1))]) && !in_array('+'. $v, $m)){unset($e[$v]);}
  }
 }
}
$C['elements'] =& $e;
...
$C['deny_attribute'] = $x;
// config URL
$x = (isset($C['schemes'][2]) && mb_strpos($C['schemes'], ':')) ? mb_strtolower($C['schemes']) : 'href: aim, feed, file, ftp, gopher, http, https, irc, mailto, news, nntp, sftp, ssh, telnet; *:file, http, https';
$C['schemes'] = array();
...
$C['parent'] = isset($C['parent'][0]) ? mb_strtolower($C['parent']) : 'body';
...
$t = (($C['cdata'] or $C['comment']) && mb_strpos($t, "\x01") !== false) ? str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05"), array('', '', '&', '<', '>'), $t) : $t;
...
for($i=-1, $ci=count($t); ++$i<$ci;){
 ...
 if(isset($x[0])){
  ...
  elseif(mb_strpos($x, "\x02\x04")){
   foreach(preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $x, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $v){
    echo (mb_substr($v, 0, 2) == "\x01\x02" ? $v : ($do > 4 ? preg_replace('`\S`', '', $v) : ''));
   }
...
if($v == 1){return '';}
if($n == 'comment'){
 if(mb_substr(($t = preg_replace('`--+`', '-', mb_substr($t, 4, -3))), -1) != ' '){$t .= ' ';}
}
else{$t = mb_substr($t, 1, -1);}
...
if(($n = ctype_digit($t = mb_substr($t, 1)) ? intval($t) : hexdec(mb_substr($t, 1))) < 9 or ($n > 13 && $n < 32) or $n == 11 or $n == 12 or ($n > 126 && $n < 160 && $n != 133) or ($n > 55295 && ($n < 57344 or ($n > 64975 && $n < 64992) or $n == 65534 or $n == 65535 or $n > 1114111))){
...
static $d = 'denied:';
if(isset($c['!']) && mb_substr($p, 0, 7) != $d){$p = "$d$p";}
if(isset($c['*']) or !strcspn($p, '#?;') or (mb_substr($p, 0, 7) == $d)){return "{$b}{$p}{$a}";} // All ok, frag, query, param
if(preg_match('`^([^:?[@!$()*,=/\'\]]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', $p, $m) && !isset($c[mb_strtolower($m[1])])){ // Denied prot
 return "{$b}{$d}{$p}{$a}";
}
if($C['abs_url']){
 if($C['abs_url'] == -1 && mb_strpos($p, $C['base_url']) === 0){ // Make url rel
  $p = mb_substr($p, mb_strlen($C['base_url']));
 }elseif(empty($m[1])){ // Make URL abs
  if(mb_substr($p, 0, 2) == '//'){$p = htmLawed . phpmb_substr($C['base_url'], 0, mb_strpos($C['base_url'], ':') + 1);}
  elseif($p[0] == '/'){$p = htmLawed . phppreg_replace('`(^.+?://[^/]+)(.*)`', '$1', $C['base_url']);}
...
// final $spec
$s = array();
$t = str_replace(array("\t", "\r", "\n", ' '), '', preg_replace_callback('/"(?>(`.|[^"])*)"/sm', create_function('$m', 'return mb_substr(str_replace(array(";", "|", "~", " ", ",", "/", "(", ")", \'`"\'), array("\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07", "\x08", "\""), $m[0]), 1, -1);'), trim($t)));
for($i = count(($t = explode(';', $t))); --$i>=0;){
 $w = $t[$i];
 if(empty($w) or ($e = mb_strpos($w, '=')) === false or !mb_strlen(($a =  mb_substr($w, $e+1)))){continue;}
 $y = $n = array();
 foreach(explode(',', $a) as $v){
  if(!preg_match('`^([a-z:\-\*]+)(?:\((.*?)\))?`i', $v, $m)){continue;}
  if(($x = mb_strtolower($m[1])) == '-*'){$n['*'] = 1; continue;}
  if($x[0] == '-'){$n[substr($x, 1)] = 1; continue;}
  if(!isset($m[2])){$y[$x] = 1; continue;}
  foreach(explode('/', $m[2]) as $m){
   if(empty($m) or ($p = mb_strpos($m, '=')) == 0 or $p < 5){$y[$x] = 1; continue;}
   $y[$x][mb_strtolower(mb_substr($m, 0, $p))] = str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05", "\x06", "\x07", "\x08"), array(";", "|", "~", " ", ",", "/", "(", ")"), mb_substr($m, $p+1));
  }
 ...
 foreach(explode(',', mb_substr($w, 0, $e)) as $v){
  if(!mb_strlen(($v = mb_strtolower($v)))){continue;}
  if(count($y)){$s[$v] = $y;}
  if(count($n)){$s[$v]['n'] = $n;}
 }
}
...
}elseif(!isset($C['elements'][($e = mb_strtolower($m[2]))])){
...
// attr name-vals
if(mb_strpos($a, "\x01") !== false){$a = preg_replace('`\x01[^\x01]*\x01`', '', $a);} // No comment/CDATA sec
 ...
 switch($mode){
  ...
  break; case 2: // Val
   if(preg_match('`^((?:"[^"]*")|(?:\'[^\']*\')|(?:\s*[^\s"\']+))(.*)`', $a, $m)){
    ...
    $aA[$nm] = trim(str_replace('<', '&lt;', ($m[0] == '"' or $m[0] == '\'') ? mb_substr($m, 1, -1) : $m));
   }
  break;
 }
...
$a = array(); $nfr = 0;
foreach($aA as $k=>$v){
  if($C['allow_attribute'] or ((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e]))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])){
  if(isset($aNE[$k])){$v = $k;}
  elseif(!empty($lcase) && (($e != 'button' or $e != 'input') or $k == 'type')){ // Rather loose but ?not cause issues
   $v = (isset($aNL[($v2 = mb_strtolower($v))])) ? $v2 : $v;
  }
  if($k == 'style' && !$C['style_pass']){
   if(false !== mb_strpos($v, '&#')){
  ...
  }elseif(isset($aNP[$k]) or mb_strpos($k, 'src') !== false or $k[0] == 'o'){
	$v = str_replace('­', ' ', (mb_strpos($v, '&') !== false ? str_replace(array('&#xad;', '&#173;', '&shy;'), ' ', $v) : $v));
	$v = hl_prot($v, $k);
   if($k == 'href'){ // X-spam
    if($C['anti_mail_spam'] && mb_strpos($v, 'mailto:') === 0){
...
// Tidy/compact HTM
if(mb_strpos(' pre,script,textarea', "$p,")){return $t;}
...
while($X){
 ...
 for($i=-1, $j=count($t); ++$i<$j;){
  ...
  $x = $e[0] == '/' ? 0 : (mb_substr($e, -1) == '/' ? 1 : ($e[0] != '!' ? 2 : -1));
  $y = !$x ? ltrim($e, '/') : ($x > 0 ? mb_substr($e, 0, strcspn($e, ' ')) : 0);
  $e = "<$e>";
  if(isset($d[$y])){
   if(!$x){
    if($n){echo "\n", str_repeat($s, --$n), "$e\n", str_repeat($s, $n);}
...
$t = str_replace(array("\n ", " \n"), "\n", preg_replace('`[\n]\s*?[\n]+`', "\n", ob_get_contents()));
ob_end_clean();
if(($l = mb_strpos(" $w", 'r') ? (mb_strpos(" $w", 'n') ? "\r\n" : "\r") : 0)){
 $t = str_replace("\n", $l, $t);
}
return str_replace(array("\x01", "\x02", "\x03", "\x04", "\x05", "\x07"), array('<', '>', "\n", "\r", "\t", ' '), $t);
// eof
...
$C['keep_bad'] = 1;
$C['elements'] = count($h) ? mb_strtolower(implode(',', array_keys($h))) : '-*';
$C['hook'] = 'kses_hook';
$C['schemes'] = '*:'. implode(',', $p);
